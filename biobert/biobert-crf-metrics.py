...
idx_to_tag = {}
for d in tag_to_ix.keys():
    a=tag_to_ix[d]
    idx_to_tag[a]=d

model = BiLSTM_CRF(tag_to_ix, EMBEDDING_DIM, HIDDEN_DIM)
model.load_state_dict(torch.load('model_checkpoint3'))
...
for sentence, tags in training_data:
    ...
    pred_tags.append([idx_to_tag[t] for t in probs])
    #break

for i in range(len(test_tags)):
    for j in range(len(test_tags[i])):
        if 'CLS' in test_tags[i][j]:
            test_tags[i][j]='CLS'
        if 'SEP' in test_tags[i][j]:
            test_tags[i][j]='SEP'

for i in range(len(pred_tags)):
    for j in range(len(pred_tags[i])):
        if 'CLS' in pred_tags[i][j]:
            pred_tags[i][j]='CLS'
        if 'SEP' in pred_tags[i][j]:
            pred_tags[i][j]='SEP'
